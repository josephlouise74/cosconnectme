            ...
            switch (stepManager.currentStep) {
                ...
                case "payment":
                    ...
                    if (missingPayment.length > 0) {
                        console.warn("Payment validation failed - missing:", missingPayment)
                        for (const field of missingPayment) {
                            await form.trigger(`payment_method.${field}` as any)
                        }
                        return false
                    }
                    ...
                    // Validate refund GCash number
                    const refundGcashNumber = currentData.payment_method.refund_gcash_number
                    if (refundGcashNumber && !/^\+63\d{10}$/.test(refundGcashNumber)) {
                        await form.trigger('payment_method.refund_gcash_number')
                        return false
                    }
                    break
                case "summary":
